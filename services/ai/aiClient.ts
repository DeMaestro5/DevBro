...
export class AIClient {
  ...
  async generateMessage(context: string, tone: string = 'encouraging'): Promise<AIResponse> {
    try {
  ...
  async generateChallenge(activity: any): Promise<any> {
    try {
      ...
      const mockChallenge = {
        ...
        requirements: ['Find 3 open source projects', 'Submit at least 1 PR', 'Write meaningful comments']
      };
